...
function getClasesClass($uri) {
    return (strpos($uri, 'clases') !== false && strpos($uri, 'explorar') === false) 
        ? 'bg-blue-600 text-white shadow-md' 
        : 'text-gray-400 hover:bg-slate-800 hover:text-white';
}
?>
...
<aside class="w-64 bg-slate-900 text-white min-h-screen p-4 sticky top-0 flex flex-col transition-all duration-300 z-20 shadow-lg">
    ...
    <div class="mb-8 pt-4 text-center">
            ...
            <p class="text-xs text-blue-300 mt-1 uppercase tracking-wider font-semibold">Portal Alumno</p>
    </div>
    ...
    <nav class="space-y-2 flex-1">
        ...
        <a href="<?= BASE_URL ?>clases" class="flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium group <?= getClasesClass($uri) ?>">
